...
function processLastItem(stringList, callback) {
  let lastIndex = (stringList.length - 1);
  let lastEle = (stringList[lastIndex]);
  return callback(lastEle);
}
...
function stringSmash(strings) {
  return strings.reduce(function(acc, item){
    return acc + item;
  })
}
...
function getFullNames(runners) {
  const names = runners.map(function(item){
...
function firstNamesAllCaps(runners) {
  const caps = runners.map(function (item){
...
function tallyUpDonations(runners) {
  let num = runners.reduce(function(accumulator, item){
